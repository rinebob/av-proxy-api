...
export class AvIntradayHandler extends AlphaVantageBaseHandler<any> {
  ...
  public async fetch(params: any = {}): Promise<ApiResponse<any>> {
    ...
    log.info('fetch.start', { endpointId: endpoint, symbol, interval, outputsize, extended_hours, requestId });

    try {
      // Include AV entitlement (always delayed for intraday)
      const raw = await this.fetchSimple({ symbol, interval, outputsize, extended_hours, entitlement: AV_ENTITLEMENT_DELAYED });
      const resp = this.createSuccessResponse(raw, this.config.ttl, startTime);
      hr('aV.Intra', `fetch ok [${requestId}] ${Date.now() - startTime}ms`);
      log.info('fetch.success', { endpointId: endpoint, symbol, interval, outputsize, extended_hours, durationMs: Date.now() - startTime, requestId });
      return resp;
    } catch (e: any) {
      hr('aV.Intra', `fetch error [${requestId}] ${String(e?.message || e)}`);
      log.error('fetch.error', { endpointId: endpoint, symbol, interval, outputsize, extended_hours, error: String(e?.message || e), requestId });
