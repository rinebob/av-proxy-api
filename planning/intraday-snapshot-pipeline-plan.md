...
| Schedule cadence? | **8am, 10am, 12pm PT Mon–Fri** | 3 captures/day. Reduced from 6 to limit update volume. Cron: `0 8,10,12 * * 1-5`. |
...
```
Cloud Scheduler (0 8,10,12 * * 1-5, PT)
  └─ refreshAvDailyTimeSeriesIntradayHourly (onSchedule)
       └─ runIntradaySnapshotJobsForSymbols()
         ...
         processIntradaySnapshotJobInternal()
            ...
            ├─ upsertAvDailyIntradaySnapshot()   ← daily trailing bar
            ├─ upsertAvWeeklyIntradaySnapshot()  ← weekly trailing bar
            ├─ upsertAvMonthlyIntradaySnapshot() ← monthly trailing bar
...
- [x] Define `IntradaySnapshotJobPayload` interface: `{ marketDate, symbol, runId, clockPt }`
- [x] Implement `processIntradaySnapshotJobInternal(payload)`:
  ...
  - Call `upsertAvWeeklyIntradaySnapshot({ symbol, marketDate, ip, io, clockPt })`
  - Call `upsertAvMonthlyIntradaySnapshot({ symbol, marketDate, ip, io, clockPt })`
  - All three writes share the same `ip`/`io` — one AV API call, three bar patches
...
- [x] Reuse `MAX_JOB_ATTEMPTS` and `JOB_EXECUTION_DELAY_MS` from `job-config.ts`
...
- [x] Implement `onIntradayRunJobTerminal({ runId, symbol, status })`:
...
- [x] Add `jobsCreationStartedAt` timestamp to the run doc (set by the scheduler runner) so the reconcile path can compute the age window correctly — same pattern as `realtime-runs`
...
- [x] Export `processIntradaySnapshotJobTask` via `onTaskDispatched<IntradaySnapshotJobPayload>`
- [x] Use same `retryConfig` and `memory` as `processTimeSeriesJobTask` (from `job-config.ts`)
- [x] Rate limits: `maxDispatchesPerSecond: 1.0`, `maxConcurrentDispatches: 20` (same as POST — tunable later)
- [x] Delegate to `processIntradaySnapshotJobInternal`

---

### Step 4 – Scheduler runner function (`av-time-series-refresh-manager.ts`)

- [x] Add `runIntradaySnapshotJobsForSymbols(options)` function:
  - Reads `tracked-symbols`
...
- [x] Fix `refreshAvDailyTimeSeriesIntradayHourly`: replace no-op `runTimeSeriesJobsForEndpoint` call with `runIntradaySnapshotJobsForSymbols()`
- [x] Remove `refreshAvDailyTimeSeriesPreClose` export and function body (no-op, confusing)
...
- [x] Add `INTRADAY_SNAPSHOT_JOB = 'processIntradaySnapshotJobTask'` to `CloudTask` enum

**File:** `functions/src/v2/alpha-vantage/jobs/job-config.ts`
- [x] Add `INTRADAY_MAX_RUN_DURATION_MS = 20 * 60 * 1000` (20 minutes) — intraday-specific reconcile window, separate from the shared `MAX_RUN_DURATION_MS` (60 min) used by the POST pipeline
...
- [x] Export `processIntradaySnapshotJobTask` from `intraday-snapshot-jobs.task`
- [x] Remove export of `refreshAvDailyTimeSeriesPreClose` (no-op, being deleted)
- [x] Remove export of `refreshAvIntradayRthClose1615Pre` (superseded)
- [x] Keep export of `refreshAvDailyTimeSeriesIntradayHourly` (now correctly wired)

---

### Step 7 – Cleanup in `av-refresh-manager.ts`
- [x] Remove or stub out `refreshAvIntradayRthClose1615Pre` (leave a deprecation comment referencing the new pipeline)
- [x] Remove `TS_INTRADAY_RTH_CLOSE_1615` schedule reference if no longer used

---

### Step 8 – Schedule constants (`function-schedules.ts`)
- [x] Remove `TS_DAILY_PRE_CLOSE_SCHEDULE` if no longer referenced
- [x] Keep `TS_DAILY_INTRADAY_HOURLY_SCHEDULE` (`0 8,10,12 * * 1-5`)
...
**Unchanged:** `av-intraday.handler.ts`, `CompactBar` type, `DayOfWeek` enum, all POST pipeline files.
