...
**Status:** Planned endpoint; do not call until Savant provides the deployed URL  
...
- Use the exact endpoint URL supplied by Savant for both the request and token audience.
...
The URL above is an expected naming pattern, not a deployment guarantee. Use the exact value Savant supplies.
...
All fields other than `type` are optional. Numeric values are strings. RS must parse them with validation and represent unavailable or invalid values as missing rather than as zero.
...
- On `429`, honor `Retry-After`; otherwise use exponential backoff with jitter.
...
| `UNAUTHORIZED` / `FORBIDDEN` | Verify service account, IAM invoker role, email allowlist, and OIDC audience. |
...
4. Confirm the token audience exactly equals the supplied endpoint URL.
