...
export class AlphaVantageDataService {
  ...
  fetchData<T = AlphaVantageResponseData>(
    ...
    console.group(`🔵 aVDSvc fD [${requestId}] AlphaVantage API Request`);
    ...
    return this.http.get<AlphaVantageApiResponse<T>>(urlWithParams, {
      ...
      tap({
        next: (response) => {
          ...
          console.group(`🟢 aVDSvc fD [${requestId}] AlphaVantage API Response (${duration}ms)`);
          console.log('Endpoint:', endpoint);
        ...
        error: (error: HttpErrorResponse) => {
          const duration = Date.now() - startTime;
          console.group(`🔴 aVDSvc fD [${requestId}] AlphaVantage API Error (${duration}ms)`);
